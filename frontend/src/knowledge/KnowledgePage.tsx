...
function EvidenceWorkspace({ onShowFactCards, onShowSearch }: { onShowFactCards: () => void; onShowSearch: () => void }) {
  ...
  const [extractorStrategy, setExtractorStrategy] = useState("manual_excerpt.v1");
  const [author, setAuthor] = useState("console_operator");
  ...
  const createSource = useMutation({
    mutationFn: () => knowledgeApi.createSourceEvidence({ source_type: sourceType, title: sourceTitle.trim(), source_url: sourceUrl.trim() || undefined, excerpt: sourceExcerpt.trim(), access_scope: sourceScope.trim(), extractor_strategy_ref: extractorStrategy.trim() || "manual_excerpt.v1", created_by: author.trim() || undefined }),
    onSuccess: async () => { setSourceTitle(""); setSourceUrl(""); setSourceExcerpt(""); setExtractorStrategy("manual_excerpt.v1"); await refresh(); },
  });
  ...
  return <div className="knowledge-layout">
    <aside className="wb-section knowledge-rail">
      ...
      <form className="knowledge-create knowledge-editor" onSubmit={(event: FormEvent) => { event.preventDefault(); createSource.mutate(); }}>
        ...
        <label className="wb-field"><span>抽取策略</span><input className="wb-input" value={extractorStrategy} onChange={(event) => setExtractorStrategy(event.target.value)} required /></label>
        <label className="wb-field"><span>登记人</span><input className="wb-input" value={author} onChange={(event) => setAuthor(event.target.value)} required /></label>
