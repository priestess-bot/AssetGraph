...
export const functionalVideosApi = {
  ...
  updateTimeline: (code: string, payload: { expected_revision: number; video_clips: Array<{ clip_code: string; duration_ms: number; transition: string; source_start_seconds?: number; source_end_seconds?: number; fit?: "cover" | "contain"; crop_x?: number; crop_y?: number; playback_rate?: number }>; subtitle_clips?: Array<{ clip_code: string; subtitle_text: string; headline_text: string }>; audio_clips?: Array<{ clip_code: string; gain_db: number }> }) => requestJson<unknown>(`${ROOT}/${code}/timeline`, { method: "PUT", body: JSON.stringify(payload) }).then(plan),
