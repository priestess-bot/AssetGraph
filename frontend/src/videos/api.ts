...
export interface FunctionalVideoPlan {
  ...
  productionTimeline: { global_end_ms: number; tracks: Array<{ track_kind: string; clips: Array<{ clip_code: string; timeline_range: { start_ms: number; duration_ms: number }; source_range?: { asset_code: string; start_seconds?: number; end_seconds?: number; available_start_seconds?: number; available_end_seconds?: number }; transition?: string; fit?: "cover" | "contain"; crop_x?: number; crop_y?: number; playback_rate?: number; overlay_roles?: string[]; linked_shot_code?: string; subtitle_text?: string; headline_text?: string; caption_position?: "bottom" | "center"; gain_db?: number }> }> };
...
export const functionalVideosApi = {
  ...
  updateTimeline: (code: string, payload: { expected_revision: number; video_clips: Array<{ clip_code: string; duration_ms: number; transition: string; source_start_seconds?: number; source_end_seconds?: number; fit?: "cover" | "contain"; crop_x?: number; crop_y?: number; playback_rate?: number; show_product_sticker?: boolean }>; subtitle_clips?: Array<{ clip_code: string; subtitle_text: string; headline_text: string; caption_position: "bottom" | "center" }>; audio_clips?: Array<{ clip_code: string; gain_db: number }> }) => requestJson<unknown>(`${ROOT}/${code}/timeline`, { method: "PUT", body: JSON.stringify(payload) }).then(plan),
  ...
  createReleaseCandidate: (code: string) => postJson<unknown>(`${ROOT}/${code}/release-candidate`, {}).then(plan),
