...
function editableSubtitleClips(subtitleTrack: FunctionalVideoPlan["productionTimeline"]["tracks"][number] | undefined): EditableSubtitleClip[] {
  return subtitleTrack?.clips.map((clip) => ({
    clipCode: clip.clip_code,
    shotCode: clip.linked_shot_code ?? clip.clip_code.replace(/^SUBTITLE-/, ""),
...
function TimelineEditor({ plan }: { plan: FunctionalVideoPlan }) {
  ...
  const update = useMutation({
    mutationFn: () => functionalVideosApi.updateTimeline(plan.planCode, {
      ...
      video_clips: clips.map((clip) => ({
        ...
        ...(typeof clip.cropX === "number" && typeof clip.cropY === "number" ? { crop_x: clip.cropX, crop_y: clip.cropY } : {}),
        ...(typeof clip.playbackRate === "number" ? { playback_rate: clip.playbackRate } : {}),
        ...(typeof clip.productSticker === "boolean" ? { show_product_sticker: clip.productSticker } : {}),
      })),
  ...
  const subtitleContentInvalid = subtitleClips.some((clip) => !clip.subtitleText.trim());
  return <section className="wb-section"><SectionHeader kicker={`TIMELINE r${plan.timelineRevision}`} title="时间轴" actions={<StatusBadge label={`${totalSeconds.toFixed(1)} 秒`} tone="info" />} /><div className="video-timeline">{clips.map((clip) => <div key={clip.clipCode} style={{ flexGrow: Math.max(1, clip.durationMs) }}><span>{clip.clipCode}</span><strong>{(clip.durationMs / 1000).toFixed(1)}s</strong><small>{videoTrack?.clips.find((item) => item.clip_code === clip.clipCode)?.source_range?.asset_code ?? "voice"} · {clip.transition}</small></div>)}</div><div className="video-timeline-editor">{clips.map((clip, index) => {
    ...
    return <div key={clip.clipCode}><code>{clip.clipCode}</code><div className="video-clip-order"><button type="button" className="wb-icon-button" title="上移镜头" aria-label={`上移 ${clip.clipCode}`} disabled={!editable || index === 0} onClick={() => moveClip(index, -1)}><ArrowUp size={14} aria-hidden="true" /></button><button type="button" className="wb-icon-button" title="下移镜头" aria-label={`下移 ${clip.clipCode}`} disabled={!editable || index === clips.length - 1} onClick={() => moveClip(index, 1)}><ArrowDown size={14} aria-hidden="true" /></button></div><label className="wb-field"><span>时长（毫秒）</span><input className="wb-input" type="number" min="250" max="120000" value={clip.durationMs} disabled={!editable} onChange={(event) => updateClip(index, { durationMs: Number(event.target.value) })} /></label><label className="wb-field"><span>转场</span><select className="wb-input" value={clip.transition} disabled={!editable} onChange={(event) => updateClip(index, { transition: event.target.value })}><option value="cut">直接切换</option><option value="fade">淡入淡出</option><option value="fade_out">淡出</option></select></label><label className="wb-field"><span>画面适配</span><select aria-label={`画面适配 ${clip.clipCode}`} className="wb-input" value={clip.fit ?? ""} disabled={!editable} onChange={(event) => { const fit = event.target.value; updateClip(index, { fit: fit === "cover" || fit === "contain" ? fit : undefined, ...(fit === "contain" ? { cropX: undefined, cropY: undefined } : {}) }); }}><option value="">沿用</option><option value="cover">cover</option><option value="contain">contain</option></select></label><label className="wb-field"><span>播放速度</span><select aria-label={`播放速度 ${clip.clipCode}`} className="wb-input" value={clip.playbackRate ?? 1} disabled={!editable} onChange={(event) => updateClip(index, { playbackRate: Number(event.target.value) })}><option value="0.5">0.5x</option><option value="0.75">0.75x</option><option value="1">1x</option><option value="1.25">1.25x</option><option value="1.5">1.5x</option><option value="2">2x</option></select></label><label className="wb-field video-overlay-toggle"><span>商品贴片</span><input aria-label={`商品贴片 ${clip.clipCode}`} type="checkbox" checked={clip.productSticker ?? false} disabled={!editable} onChange={(event) => updateClip(index, { productSticker: event.target.checked })} /></label>{cropEnabled ? <div className="video-crop-position"><label className="wb-field"><span>裁切焦点 X {Math.round((clip.cropX ?? 0.5) * 100)}%</span><input aria-label={`裁切焦点 X ${clip.clipCode}`} type="range" min="0" max="100" step="1" value={Math.round((clip.cropX ?? 0.5) * 100)} disabled={!editable} onChange={(event) => updateClip(index, { cropX: Number(event.target.value) / 100, cropY: clip.cropY ?? 0.5 })} /></label><label className="wb-field"><span>裁切焦点 Y {Math.round((clip.cropY ?? 0.5) * 100)}%</span><input aria-label={`裁切焦点 Y ${clip.clipCode}`} type="range" min="0" max="100" step="1" value={Math.round((clip.cropY ?? 0.5) * 100)} disabled={!editable} onChange={(event) => updateClip(index, { cropX: clip.cropX ?? 0.5, cropY: Number(event.target.value) / 100 })} /></label></div> : null}{hasSourceRange ? <div className="video-source-range"><label className="wb-field"><span>素材入点（秒）</span><input aria-label={`素材入点 ${clip.clipCode}`} className="wb-input" type="number" step="0.01" min={sourceLowerBound} max={clip.sourceEndSeconds! - 0.01} value={clip.sourceStartSeconds} disabled={!editable} onChange={(event) => updateClip(index, { sourceStartSeconds: Number(event.target.value) })} /></label><label className="wb-field"><span>素材出点（秒）</span><input aria-label={`素材出点 ${clip.clipCode}`} className="wb-input" type="number" step="0.01" min={clip.sourceStartSeconds! + 0.01} max={sourceUpperBound} value={clip.sourceEndSeconds} disabled={!editable} onChange={(event) => updateClip(index, { sourceEndSeconds: Number(event.target.value) })} /></label><small>固定可用范围 {sourceLowerBound.toFixed(2)}-{sourceUpperBound?.toFixed(2) ?? "--"} 秒</small></div> : null}</div>;
  })}</div>{audioClips.length ? <div className="video-audio-editor"><header><strong>配音轨</strong><small>增益会随对应镜头固定，重新渲染时写入音频处理。</small></header>{audioClips.map((clip, index) => <label key={clip.clipCode} className="wb-field"><span>{clip.shotCode} · {clip.gainDb.toFixed(1)} dB</span><input aria-label={`配音增益 ${clip.shotCode}`} type="range" min="-24" max="12" step="0.5" value={clip.gainDb} disabled={!editable} onChange={(event) => updateAudio(index, { gainDb: Number(event.target.value) })} /></label>)}</div> : null}{subtitleTrack ? <div className="video-subtitle-editor"><header><strong>字幕轨</strong><small>字幕与标题始终绑定固定镜头，并随镜头时间范围同步。</small></header>{subtitleClips.map((clip, index) => <div key={clip.clipCode}><code>{clip.shotCode}</code><label className="wb-field"><span>字幕文本</span><textarea aria-label={`字幕文本 ${clip.shotCode}`} className="wb-input" rows={2} maxLength={500} value={clip.subtitleText} disabled={!editable} onChange={(event) => updateSubtitle(index, { subtitleText: event.target.value })} /></label><label className="wb-field"><span>标题文本</span><input aria-label={`标题文本 ${clip.shotCode}`} className="wb-input" maxLength={160} value={clip.headlineText} disabled={!editable} onChange={(event) => updateSubtitle(index, { headlineText: event.target.value })} /></label><label className="wb-field"><span>字幕位置</span><select aria-label={`字幕位置 ${clip.shotCode}`} className="wb-input" value={clip.captionPosition} disabled={!editable} onChange={(event) => updateSubtitle(index, { captionPosition: event.target.value === "center" ? "center" : "bottom" })}><option value="bottom">底部</option><option value="center">居中</option></select></label></div>)}</div> : null}{subtitleContentInvalid ? <InlineNotice tone="warning" title="字幕文本不能为空">每个固定镜头均需保留一段字幕文本。</InlineNotice> : null}<TimelineRevisionHistory plan={plan} />{editable ? <div className="wb-form-actions"><button type="button" className="wb-button wb-button-primary" disabled={update.isPending || totalSeconds < 30 || totalSeconds > 120 || subtitleContentInvalid} onClick={() => update.mutate()}>保存时间轴修订</button></div> : <InlineNotice tone="warning" title="时间轴已锁定">渲染任务已锁定。请创建新的成片分支进行调整。</InlineNotice>}{update.error ? <InlineNotice tone="danger" title="时间轴未保存">{text(update.error)}</InlineNotice> : null}</section>;
}
...
export function VideoProductionPage() {
  const queryClient = useQueryClient(); const [sourceKind, setSourceKind] = useState<"project" | "live_room">("project"); const [projectCode, setProjectCode] = useState(""); const [liveRoomPlanCode, setLiveRoomPlanCode] = useState(""); const [duration, setDuration] = useState(55); const [selected, setSelected] = useState("");
  const projects = useQuery({ queryKey: ["content-projects"], queryFn: contentProjectsApi.list }); const liveRoomPlans = useQuery({ queryKey: ["functional-live-rooms"], queryFn: functionalLiveRoomsApi.list }); const plans = useQuery({ queryKey: ["functional-videos"], queryFn: functionalVideosApi.list });
  useEffect(() => { if (!projectCode && projects.data?.[0]) setProjectCode(projects.data[0].projectCode); }, [projectCode, projects.data]);
  useEffect(() => { if (!liveRoomPlanCode && liveRoomPlans.data?.[0]) setLiveRoomPlanCode(liveRoomPlans.data[0].planCode); }, [liveRoomPlanCode, liveRoomPlans.data]);
  ...
  const sourceReady = sourceKind === "project" ? Boolean(projectCode) : Boolean(liveRoomPlanCode);
  const create = useMutation({ mutationFn: () => functionalVideosApi.create(sourceKind === "project" ? { project_code: projectCode, target_duration_seconds: duration } : { live_room_plan_code: liveRoomPlanCode, target_duration_seconds: duration }), onSuccess: (plan) => { setSelected(plan.planCode); void queryClient.invalidateQueries({ queryKey: ["functional-videos"] }); } });
  const submit = (event: FormEvent) => { event.preventDefault(); if (sourceReady) create.mutate(); };
  return <div className="video-plan-layout"><aside className="wb-section video-plan-rail"><SectionHeader kicker="RENDERED VIDEO" title="成片生产" /><form className="video-plan-form" onSubmit={submit}><label className="wb-field"><span>内容来源</span><select aria-label="内容来源" className="wb-input" value={sourceKind} onChange={(event) => setSourceKind(event.target.value as "project" | "live_room")}><option value="project">内容项目</option><option value="live_room">直播间计划</option></select></label>{sourceKind === "project" ? <label className="wb-field"><span>内容项目</span><select className="wb-input" value={projectCode} onChange={(event) => setProjectCode(event.target.value)}>{projects.data?.map((project) => <option key={project.projectCode} value={project.projectCode}>{project.title}</option>)}</select></label> : <label className="wb-field"><span>直播间计划</span><select aria-label="直播间计划" className="wb-input" value={liveRoomPlanCode} onChange={(event) => setLiveRoomPlanCode(event.target.value)}>{liveRoomPlans.data?.map((plan) => <option key={plan.planCode} value={plan.planCode}>{plan.expectedTitle} · {plan.planCode}</option>)}</select></label>}<label className="wb-field"><span>目标时长（秒）</span><input className="wb-input" type="number" min="30" max="120" value={duration} onChange={(event) => setDuration(Number(event.target.value))} /></label><button className="wb-button wb-button-primary" disabled={!sourceReady || create.isPending}><Send size={15} aria-hidden="true" />创建渲染任务</button>{create.error ? <InlineNotice tone="danger" title="无法创建成片任务">{text(create.error)}</InlineNotice> : null}</form><div className="video-plan-list">{plans.data?.map((plan) => <button type="button" key={plan.planCode} className={plan.planCode === active ? "active" : undefined} onClick={() => setSelected(plan.planCode)}><span><strong>{plan.title}</strong><code>{plan.planCode}</code><small>{plan.progressPercent}% · {plan.currentStage ?? "queued"}</small></span><StatusBadge label={jobLabel(plan.jobStatus)} tone={jobTone(plan.jobStatus)} /></button>)}</div></aside><main className="video-plan-main">{plans.isLoading || projects.isLoading || liveRoomPlans.isLoading ? <LoadingBlock /> : detail.data ? <Detail plan={detail.data} onBranchCreated={(next) => setSelected(next.planCode)} /> : <EmptyBlock icon={Film} title="创建第一个成片任务" detail="内容项目或其直播间计划会被编译为垂直视频时间轴并提交给渲染 Worker。" />}</main></div>;
}
