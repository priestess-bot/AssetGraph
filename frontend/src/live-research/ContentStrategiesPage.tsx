...
export function ContentStrategiesPage() {
  ...
  return <div className="research-strategy-layout">
    ...
    <main className="research-strategy-main">{loading ? <LoadingBlock label="正在读取来源直播间与模板" /> : problem ? <InlineNotice tone="danger" title="内容策略工作台无法加载">{errorMessage(problem)}</InlineNotice> : <>{demoMode ? <InlineNotice tone="warning" title="当前展示内容策略演示数据">连接直播研究服务后会读取真实来源场次、时间线和已发布策略。</InlineNotice> : null}
      ...
      {active ? <section className="wb-section"><SectionHeader kicker={`${active.template_code} · r${active.latest_revision}`} title={active.title} actions={<><StatusBadge label={active.contentReadiness === "ready" ? "内容可用" : active.contentReadiness} tone={readinessTone(active.contentReadiness)} /><StatusBadge label="reference_only" tone="info" /></>} /><div className="wb-section-body research-strategy-detail"><div><span>来源直播间</span><code>{active.sourceTargetCode}</code></div><div><span>适用品类</span><strong>{active.contentStrategy.targetCategory || "待加载"}</strong></div><div><span>素材提示</span><strong>{active.contentStrategy.materialCues.join(" / ") || "未声明"}</strong></div><section className="research-strategy-outline"><h3>节目结构与媒体证据</h3>{active.contentStrategy.programOutline.map((module) => <article key={module.moduleKey}><code>{module.moduleKey}</code><strong>{module.title}</strong><span>{module.purpose}</span><small>{Math.round((module.endMs - module.startMs) / 1000)} 秒</small><a href={`/research/live-sources?view=sessions&session=${encodeURIComponent(module.sourceSessionCode ?? "")}`} title="打开来源录屏场次"><ExternalLink size={13} aria-hidden="true" />{module.sourceSessionCode ?? "缺少来源证据"}</a></article>)}</section>{active.contentStrategy.reviewedExamples.length ? <section className="research-strategy-examples"><h3>清洗后例证</h3>{active.contentStrategy.reviewedExamples.map((example, index) => <article key={`${example.moduleKey}-${index}`}><code>{example.moduleKey}</code><span>{example.exampleText}</span><small>{example.sourceSessionCode} · {example.startMs / 1000}-{example.endMs / 1000} 秒</small></article>)}</section> : null}{active.published_revision ? <InlineNotice title="发布版本已固定"><CheckCircle2 size={14} aria-hidden="true" />该策略可作为内容项目的主模板或次要模板，不能作为可执行布局。</InlineNotice> : <button type="button" className="wb-button wb-button-primary" disabled={publish.isPending || active.contentReadiness !== "ready"} onClick={() => publish.mutate(active)}><Send size={15} aria-hidden="true" />发布内容策略模板</button>}{publish.error ? <InlineNotice tone="danger" title="策略模板发布失败">{errorMessage(publish.error)}</InlineNotice> : null}</div></section> : <section className="wb-section"><EmptyBlock icon={CircleAlert} title="创建或选择内容策略模板" /></section>}</>}</main>
  </div>;
}
