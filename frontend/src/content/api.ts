...
export interface ContentProjectDetail extends ContentProjectSummary {
  ...
  templateContributionDecisions: Array<{ templateCode: string; revision: number; selectionRole: string; availableModules: string[]; acceptedModules: string[]; rejectedModules: string[]; materialCues: string[]; contentStrategyPolicy: Record<string, unknown> }>;
  factCards: Array<{ fact_card_code: string; version_number: number; version_code: string; content_sha256: string }>;
  factClaims: Array<{ claim_code: string; fact_code: string; source_evidence_code: string; claim: string; citation_excerpt: string; fingerprint_sha256: string }>;
...
function detail(value: unknown): ContentProjectDetail {
  ...
  const factClaims = asArray(value.fact_claims).flatMap((item) => isRecord(item) ? [{ claim_code: asString(item.claim_code), fact_code: asString(item.fact_code), source_evidence_code: asString(item.source_evidence_code), claim: asString(item.claim), citation_excerpt: asString(item.citation_excerpt), fingerprint_sha256: asString(item.fingerprint_sha256) }] : []).filter((item) => Boolean(item.claim_code && item.fingerprint_sha256));
  ...
  return { ...base, content, templateContributionDecisions, factCards, factClaims, designBrief, generated: value.generated === true, generationMode: asOptionalString(value.generation_mode), storyBrief, script, program, shotList };
}
