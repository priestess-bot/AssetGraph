...
function PacksTab({ groups, packs }: { groups: Awaited<ReturnType<typeof assetLibraryApi.listGroups>>; packs: MaterialPack[] }) {
  ...
  const publish = useMutation({ mutationFn: (packCode: string) => assetLibraryApi.publishPack(packCode), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["assets", "packs"] }); } });
  ...
  <section className="wb-section"><SectionHeader kicker="RESOLVED PREVIEW" title="解析预览" />{packs.length ? <div className="asset-summary-list">{packs.map((pack) => <div key={pack.packCode}><span><strong>{pack.title}</strong><code>{pack.packCode} · r{pack.revisionNumber}</code><small>{pack.resolvedAssetCodes.join("、") || "没有可用素材"}</small></span><span className="asset-pack-actions"><StatusBadge label={pack.status === "published" ? "已发布" : "草稿"} tone={pack.status === "published" ? "success" : "warning"} />{pack.status === "draft" ? <button type="button" className="wb-button" disabled={publish.isPending} onClick={() => publish.mutate(pack.packCode)}>发布</button> : null}</span></div>)}</div> : <EmptyBlock icon={PackagePlus} title="尚无素材包" detail={groups.length ? "可将一个或多个分组加入素材包。" : "先创建素材分组或填写素材编码。"} />}{publish.error ? <InlineNotice tone="danger" title="素材包发布失败">{message(publish.error)}</InlineNotice> : null}</section></div>;
}
