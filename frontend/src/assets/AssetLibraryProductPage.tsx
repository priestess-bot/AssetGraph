...
const MATERIAL_ROLES: MaterialRole[] = ["background", "product_display", "digital_human", "brand_title", "promotion_text", "decoration_foreground", "supporting_video", "voice", "background_music", "sound_effect"];
const CAPABILITIES: ExecutionCapability[] = ["maitu_bound", "local_only", "reference_only", "unavailable", "unclassified"];
...
function AssetImportDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [capability, setCapability] = useState<ExecutionCapability>("local_only");
  ...
  return <Dialog.Root open={open} onOpenChange={(value) => { if (!value) onClose(); }}><Dialog.Portal><Dialog.Overlay className="product-dialog-overlay" /><Dialog.Content className="asset-import-dialog"><header><div><Dialog.Title>导入素材</Dialog.Title><Dialog.Description>选择文件并明确它在直播间或成片中的用途。</Dialog.Description></div><Dialog.Close className="product-icon-button" title="关闭"><X size={18} aria-hidden="true" /></Dialog.Close></header><div className="asset-import-body"><label className={`asset-dropzone ${file ? "has-file" : ""}`}><input type="file" accept="image/*,video/*,audio/*,.pdf,.txt" onChange={(event) => { const next = event.target.files?.[0]; setFile(next); if (next && !title) setTitle(next.name.replace(/\.[^.]+$/, "")); }} /><Upload size={24} aria-hidden="true" /><strong>{file ? file.name : "选择图片、视频、音频或文档"}</strong><span>{file ? `${(file.size / 1024 / 1024).toFixed(1)} MB` : "单次导入一份素材，上传后可继续批量操作"}</span></label><label className="wb-field"><span>素材名称</span><input className="wb-input" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="用于搜索和选择的名称" /></label><div className="asset-import-options"><div><span>素材用途</span><div>{MATERIAL_ROLES.map((role) => <button type="button" key={role} className={roles.includes(role) ? "selected" : undefined} onClick={() => setRoles((current) => toggle(current, role))}>{roles.includes(role) ? <Check size={12} aria-hidden="true" /> : null}{productLabel(role, "其他用途")}</button>)}</div></div><label className="wb-field"><span>可用于</span><select className="wb-input" value={capability} onChange={(event) => setCapability(event.target.value as ExecutionCapability)}>{CAPABILITIES.map((value) => <option key={value} value={value}>{productLabel(value)}</option>)}</select></label></div>{importAsset.error ? <InlineNotice tone="danger" title="素材没有导入">请选择有效文件并确认服务连接。</InlineNotice> : null}</div><footer><button className="wb-button" type="button" onClick={onClose}>取消</button><button className="wb-button wb-button-primary" type="button" disabled={!file || !title.trim() || importAsset.isPending} onClick={() => importAsset.mutate()}>{importAsset.isPending ? "正在上传" : "导入素材"}</button></footer></Dialog.Content></Dialog.Portal></Dialog.Root>;
}

interface Geometry { x: number; y: number; width: number; height: number; preserveAspect: boolean; layer: "normal" | "top" | "bottom"; onTable: boolean }
const DEFAULT_GEOMETRY: Geometry = { x: 0.1, y: 0.1, width: 0.8, height: 0.8, preserveAspect: true, layer: "normal", onTable: false };

function geometryFromRules(rules: ConstraintRule[]): Geometry {
  const region = rules.find((rule) => rule.kind === "allowed_region")?.parameters ?? {};
  return {
    x: typeof region.x === "number" ? region.x : DEFAULT_GEOMETRY.x,
    y: typeof region.y === "number" ? region.y : DEFAULT_GEOMETRY.y,
    width: typeof region.width === "number" ? region.width : DEFAULT_GEOMETRY.width,
    height: typeof region.height === "number" ? region.height : DEFAULT_GEOMETRY.height,
    preserveAspect: rules.some((rule) => rule.kind === "preserve_aspect_ratio"),
    layer: rules.some((rule) => rule.kind === "pin_layer_top") ? "top" : rules.some((rule) => rule.kind === "pin_layer_bottom") ? "bottom" : "normal",
    onTable: rules.some((rule) => rule.kind === "require_named_region" && rule.parameters.region === "table_surface"),
  };
}

function rulesFromGeometry(value: Geometry): ConstraintRule[] {
  return [
    { kind: "allowed_region", hard: true, parameters: { x: value.x, y: value.y, width: value.width, height: value.height } },
    ...(value.preserveAspect ? [{ kind: "preserve_aspect_ratio", hard: true, parameters: {} }] : []),
    ...(value.layer === "top" ? [{ kind: "pin_layer_top", hard: true, parameters: {} }] : value.layer === "bottom" ? [{ kind: "pin_layer_bottom", hard: true, parameters: {} }] : []),
    ...(value.onTable ? [{ kind: "require_named_region", hard: true, parameters: { region: "table_surface" } }, { kind: "above_role", hard: true, parameters: { role: "background" } }] : []),
  ];
}
...
function AssetInspector({ asset, onClose }: { asset: LibraryAsset; onClose: () => void }) {
  ...
  useEffect(() => { setMediaKind(asset.mediaKind ?? ""); setRoles(asset.materialRoles as MaterialRole[]); setCapability(asset.executionCapability); setRights(asset.rightsStatus); setRightsNote(asset.rightsNote ?? ""); }, [asset]);
  const refresh = async () => queryClient.invalidateQueries({ queryKey: ["assets"] });
  const classification = useMutation({ mutationFn: () => assetLibraryApi.updateClassification(asset.assetCode, { media_kind: mediaKind || undefined, material_roles: roles, execution_capability: capability }), onSuccess: refresh });
