...
@router.patch("/{project_code}/design-brief", response_model=ContentProjectDetail)
def revise_design_brief(
    ...
    try:
        return service.revise_design_brief(
            project_code,
            expected_revision=payload.expected_revision,
...
@router.post("/{project_code}/script-revision", response_model=ContentProjectDetail)
def revise_script(
    project_code: str,
    payload: ScriptRevisionCreate,
    service: Annotated[FunctionalContentService, Depends(get_service)],
) -> dict:
    try:
        return service.revise_script(
            project_code,
            expected_revision=payload.expected_revision,
