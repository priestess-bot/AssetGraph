-- Immutable local snapshots used by a functional live-room release candidate.
-- The snapshot content remains in PostgreSQL so the fast-track workflow can
-- create a reviewable candidate without requiring an external artifact store.

CREATE TABLE IF NOT EXISTS functional_live_room_plan_release_snapshots (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    plan_id UUID NOT NULL UNIQUE REFERENCES functional_live_room_plans(id) ON DELETE CASCADE,
    ...
    CONSTRAINT chk_functional_live_room_release_snapshot_sha
        CHECK (snapshot_fingerprint_sha256 ~ '^[0-9a-f]{64}$'),
    CONSTRAINT chk_functional_live_room_release_snapshot_object
...
ALTER TABLE functional_live_room_plans
    ADD CONSTRAINT chk_functional_live_room_release_manifest_sha
        CHECK (
