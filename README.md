...
- 本地 Qwen3 embedding/reranker：通过 `D:/AI-Models/qwen3-service` 共享 HTTP 服务接入 `Qwen3-Embedding-4B` 与 `Qwen3-Reranker-4B`，AssetGraph 后端提供 `/api/rag/embeddings`、`/api/rag/rerank`、`/api/rag/qwen3/health`、基于本地 embedding artifact 的 `/api/assets/candidates` 和槽位上下文推荐 `/api/maitu/slots/{slot_code}/candidate-assets?semantic=true`
...
- 本地 Qwen3 检索接口：`GET /api/rag/qwen3/health`、`POST /api/rag/embeddings`、`POST /api/rag/rerank`、`GET /api/assets/candidates`、`GET /api/maitu/slots/{slot_code}/candidate-assets?semantic=true`，默认连接 `http://127.0.0.1:8010` 的 D 盘共享模型服务。
