...
Browser-use worker:
  ...
  - writes succeeded / released / manual_required execution receipts back to AssetGraph
```
...
The Stage 6B retry protocol gives every claim a server-generated UUID `claim_token` and monotonic `lease_version`. `BrowserUseWorker.run_once()` refuses dry-run before claiming, so the public Worker API cannot mutate queue state in simulation mode; read-only dry-runs use direct plan/build-plan paths. The worker validates the lease with a synchronous heartbeat before any Maitu operation and again before each concrete browser command, renews it in the background during long execution, and installs a cooperative guard that stops the concrete Maitu executor between side effects after heartbeat loss. Real queue execution is sealed to the exact built-in `MaituBrowserUseExecutor` backed by the exact built-in `BrowserUseCliSession`; structural Protocol implementations, subclasses, third-party executors, arbitrary sessions, instance-level method replacements, injected command runners, and a distinct asset client are rejected before claim, heartbeat, or browser activity because they could lie about guard/timeout/checkpoint enforcement. Every critical executor/session method must remain bound to its original class implementation, and executor asset reads must use the same AssetGraph client instance as the Worker. The trusted executor must expose a bounded side-effect timeout below 80% of the lease TTL; otherwise the claim is rejected before browser execution. Invalid zero, negative, NaN, or infinite heartbeat intervals fail configuration instead of becoming a request storm. After browser execution the worker cleanly stops the execution heartbeat, performs a final synchronous renewal, then runs a fresh heartbeat through execution-result confirmation. Callback request timeouts and all three attempts are capped to a total budget below the renewed lease TTL. Heartbeat and execution-result callbacks must match the current worker/token/version and an unexpired `in_progress` claim. Succeeded, manual-required, and recoverable-release outcomes all use a UUID `retry_execution_id`; recoverable failures write `retry_execution_status=released`, atomically return to `pending` without consuming an attempt, and retry uncertain HTTP responses with the same ID/payload. The backend stores the receipt and task transition atomically, so a committed response loss is safely confirmable. Generic retry-task PATCH can update metadata only while no worker lease is active; lease-owned status, attempt count, and execution identity are writable only through the fenced callback protocol. Claim returns all required queue context in the same transaction, and claim tokens are returned only by claim endpoints, not ordinary retry-task reads.

Phase 6C-A adds a lease-fenced operation checkpoint barrier around every retry side effect. Each retry operation must carry a canonical `operation_key`, authoritative SHA-256 fingerprint, and `status=ready`; `begin` must return a confirmed `execute` before mutation, completed checkpoints return `skip`, and ambiguous prior attempts return `reconcile` with zero mutation. Mutating session methods must return authoritative `verified=true` evidence before `complete`. Begin/complete transport retries reuse stable UUIDs and identical payloads, downstream operations stop if completion remains uncertain, and project save is an explicit operation rather than an implicit executor tail step. The production queue mutation methods in `BrowserUseCliSession` remain fail-closed until authoritative Maitu readback implementations are wired.

Phase 6C-B closes the manual reconciliation loop. An authenticated, server-identified operator may resolve `reconcile_required` only after the worker lease has ended and only with authoritative, secret-free evidence bound to the exact uncertain `expected_attempt_id`. `confirmed_completed` records an audited reconciliation completion so the next claim skips the already-applied operation; `confirmed_not_applied` creates a one-shot `retry_authorized` state that the next lease atomically consumes at `begin`. A stable `reconciliation_id` and payload fingerprint make duplicate submissions idempotent, while reuse with different content returns conflict. Both outcomes return the task to `pending`; no reconciliation request carries or persists a claim token.
