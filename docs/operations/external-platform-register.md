...
| `EXT-P1-RELEASE-EVIDENCE` | Actual remote draft write and authoritative readback | Target account/room authorization, approved API or browser route, empty-draft identity/readback contract and evidence-retention owner | Draft delivery, authoritative readback and reconciliation | BuildPlan generation, local draft projection and fake adapter contract tests |
| `EXT-P2-MAITU-DRAFT-WORKER` | Closing fast-track `FT-2103` against a real Maitu room | Target-scoped Worker credential, one disposable empty non-live draft, the current Worker API/UI contract, a source-Plan fingerprint handoff and final readback evidence | A demonstrated remote empty-draft write and final readback | Worker handoff API, checkpoint protocol, local execution-status projection and contract fixtures |
