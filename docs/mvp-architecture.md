...
职责：为麦兔项目/场景的一组槽位生成完整替换方案。方案 item 记录槽位编号、选中素材编号、素材标题、匹配分、匹配原因、替换策略和 `selected/missing` 状态。`browser-use-operations` 接口把方案转换为 Browser use 可理解的页面操作步骤，供 Browser use 实际操作麦兔软件。`execution-results` 接口接收 Browser use 执行结果回写，记录总体状态、单槽位操作状态、失败类型、是否可重试、重试指令、错误信息、截图素材编号和执行摘要。`retry-queue` 批量返回 pending、retryable、未超过最大尝试次数的重试任务及麦兔上下文。`retry-queue/claim-next` 原子领取下一条可执行任务，签发 `claim_token` 并递增 `lease_version`。`retry-queue/reclaim-expired` 将锁过期的 `in_progress` 任务释放回 `pending`。`retry-worker/next` 是 Browser use worker 的一站式取活入口：先回收过期任务，再领取下一条任务，并返回 Browser use 操作计划。`retry-tasks` 接口用于查询可重试失败任务；通用 PATCH 只能在无活动 lease 时修改结果摘要/重试说明，不能修改状态、尝试次数或执行编号。`retry-tasks/{retry_task_code}/browser-use-operations` 将单个重试任务转换为最小化 Browser use 重试步骤，只重试失败槽位并保持麦兔原布局。`retry-tasks/{retry_task_code}/execution-results` 通过当前租约身份和幂等 receipt 原子更新重试任务状态、重试次数、最近执行编号、截图和结果摘要；`retry_execution_status=released` 将可恢复失败幂等放回 `pending` 且不增加尝试次数，供自动 Worker 使用。`retry-tasks/{retry_task_code}/release` 只接受当前未过期租约，清空 claim 字段并将任务放回 `pending`，保留给显式人工/管理操作。Phase 6C-A 的 operation plan 将副作用显式拆成稳定 key/fingerprint 的 `primary` 与 `save_project`；lease-fenced begin/complete checkpoint 保证已完成操作由新 lease 跳过，旧 lease 未确认操作在过期回收、显式 release 或非成功 execution result 时进入 `reconcile_required`，且 `succeeded` 只有在当前 authoritative fingerprints 全部完成后才能写入。Checkpoint 不持久化 claim token，完成证据必须是 `verified=true` 且通过 Worker、Backend 与数据库约束；活动 lease 期间，关联 slot 的 PATCH/DELETE 被拒绝，以冻结外部副作用窗口内的 authoritative intent。Phase 6C-B 的 `/reconcile` 只在无活动 lease 时接受权威读回：确认已执行会形成带审计 receipt 的 reconciliation completion，确认未执行会签发一次性 `retry_authorized`，由下一 lease 的 begin 原子消费；稳定 reconciliation ID 保证重复请求幂等，receipt 不保存 claim token。
