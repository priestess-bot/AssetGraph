...
| `FT-2103` | preparation, item remains open | current implementation batch | migration `096_functional_live_room_execution_readback.sql`; confirmed plans now freeze a compact Worker handoff (`BuildPlan` code, target, checkpoint contract, source fingerprint and operation summary), and `POST /functional-live-room-plans/{plan_code}/sync-execution` projects the existing fenced script-layout execution result as requested/running/reconcile-required/failed/completed. Both the handoff API and the generic Worker start endpoint require the plan-local explicit confirmation, while standalone legacy BuildPlans retain their current protocol. The Live Room workspace exposes the status and a manual readback sync action; no endpoint can enable go-live. Unit coverage validates final-readback projection; the PostgreSQL journey validates persisted handoff fields when a migrated local test database is available. | `CHK-1202`-`CHK-1226`；the only unperformed portion is the actual remote write/readback. `EXT-P2-MAITU-DRAFT-WORKER` records the target-scoped credential, disposable empty room and final readback required to check this item. |
